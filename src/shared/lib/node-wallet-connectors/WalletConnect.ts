...
function isSignAndSendTransactionError(
  obj: any,
): obj is SignAndSendTransactionError {
  ...
  switch (type) {
    case AccountTransactionType.InitContract: {
      ...
      if (initContractPayload.param) {
        throw new Error(
          "'param' field of 'InitContract' parameters must be empty",
        );
      }
      return {
        ...payload,
        param: serializeInitContractParam(
    ...
    case AccountTransactionType.Update: {
      ...
      const [contractName, entrypointName] =
        updateContractPayload.receiveName.split('.');
      return {
        ...payload,
    ...
    default: {
      if (typedParams) {
        throw new Error(
          `'typedParams' must not be provided for transaction of type '${type}'`,
        );
      }
      return payload;
    }
...
export class WalletConnectConnection implements WalletConnection {
  ...
  getConnectedAccount() {
    ...
    const fullAddress =
      this.session.namespaces[WALLET_CONNECT_SESSION_NAMESPACE].accounts[0];
  ...
  async signAndSendTransaction(
    ...
    const params = {
      ...
      payload: accountTransactionPayloadToJson(
        serializePayloadParameters(type, payload, typedParams),
      ),
  ...
  async signMessage(accountAddress: string, msg: SignableMessage) {
    switch (msg.type) {
      ...
      case 'BinaryMessage':
        throw new Error(
          "signing 'BinaryMessage' is not yet supported by the mobile wallets",
        );
      default:
...
export class WalletConnectConnector implements WalletConnector {
  ...
  constructor(
    ...
    client.on('session_event', ({topic, params: {chainId, event}, id}) => {
      console.debug('WalletConnect event: session_event', {
        topic,
        id,
        chainId,
        event,
      ...
      if (!connection) {
        console.error(
          `WalletConnect event 'session_event' received for unknown topic '${topic}'.`,
        );
        return;
      }
    });
    client.on('session_update', ({topic, params}) => {
      ...
      if (!connection) {
        console.error(
          `WalletConnect event 'session_update' received for unknown topic '${topic}'.`,
        );
        return;
      }
    ...
    client.on('session_delete', ({topic}) => {
      ...
      if (!connection) {
        console.error(
          `WalletConnect event 'session_delete' received for unknown topic '${topic}'.`,
        );
        return;
      }
  ...
  async connect() {
    ...
    const jsonRpcClient = jsonRpcUrl
      ? new JsonRpcClient(new HttpProvider(jsonRpcUrl))
      : undefined;
