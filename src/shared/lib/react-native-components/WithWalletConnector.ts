 ...
 */
export function ephemeralConnectorType(
...
export function persistentConnectorType(
  ...
  const connectorPromises = new Map<
    WithWalletConnector,
    Map<Network, Promise<WalletConnector>>
  >();
  return {
    activate: (component: WithWalletConnector, network: Network) => {
      const delegateConnectorPromises =
        connectorPromises.get(component) ||
        new Map<Network, Promise<WalletConnector>>();
      connectorPromises.set(component, delegateConnectorPromises);
      const connectorPromise =
        delegateConnectorPromises.get(network) || create(component, network);
...
export class WithWalletConnector
  ...
  implements WalletConnectionDelegate
{
    ...
    if (type) {
      type
        ...
        .then((connector: WalletConnector) => {
          console.log('WithWalletConnector: setting active connector', {
            connector,
          });
  ...
  onAccountChanged = (
    ...
    this.setState(state => ({
      ...state,
      connectedAccounts: updateMapEntry(
  ...
  onConnected = (connection: WalletConnection, address: string | undefined) => {
    console.debug("WithWalletConnector: calling 'onConnected'", {
      connection,
      state: this.state,
    });
